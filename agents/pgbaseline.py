...
class PGBaseline:
    ...
    def update(self, rewards, log_probs, saved_states):
        ...
        # Step 3: Calculate advantages by subtracting state values from discounted rewards.
        # Detach state values from the graph to prevent gradients from flowing into the value network during the policy update.
        advantages = discounted_rewards - state_values.detach()
    ...
    def save(self, path):
        torch.save(self.policy_net.state_dict(), path + 'pgbaselin_checkpoint.pth')

    def load(self, path):
        self.policy_net.load_state_dict(torch.load(path + 'pgbaselin_checkpoint.pth'))
